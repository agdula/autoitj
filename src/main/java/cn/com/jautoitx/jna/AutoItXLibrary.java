...
public interface AutoItXLibrary extends Library {
	 ...
	 *
	 * @param clip The text to write to the clipboard.
	 */
	 ...
	 * @param button    The button to click, "left", "right" or "middle". Default
	 *                  is the left button.
	 * @param numClicks The number of times to click the mouse. Default is 1.
	 * @param x         The x position to click within the control. Default is
	 *                  center.
	 * @param y         The y position to click within the control. Default is
	 *                  center.
	 ...
	 * @param extra   Additional parameter required by some commands; use "" if
	 ...
	 * @param control The control to interact with.
	 * @param command The command to send to the control.
	 * @param extra1  Additional parameter required by some commands; use "" if
	 *                parameter is not required.
	 * @param extra2  Additional parameter required by some commands; use "" if
	 *                parameter is not required.
	 * @param result
	 * @param bufSize
	 */
    ...
    public void AU3_ControlGetPos(final WString title, final WString text,
								  final WString control, final WinDef.RECT rect);
	 ...
	 * @param text     The text of the window to access.
	 * @param control  The control to interact with.
	 * @param sendText String of characters to send to the control.
	 * @param mode     Changes how "keys" is processed: flag = 0 (default), Text
	 ...
	 * @param text        The text of the window to access.
	 * @param control     The control to interact with.
	 * @param controlText The new text to be set into the control.
	 * @return Returns 1 if success, returns 0 if window/control is not
	 * found.
	 */
	 ...
	 * @param share    The remote share to connect to in the form
	 *                 "\\server\share".
	 ...
	 * @param user     The username to use to connect. In the form "username" or
	 *                 "domain\Username".
	 ...
	 *
	 * @param device  The device (drive or printer) letter to query. Eg. "O:" or
	 *                "LPT1:"
	 * @param mapping
	 * @param bufSize
	 */
	 ...
	 * @param x      The x coordinate to move the mouse to. If no x and y
	 *               coords are given, the current position is used.
	 * @param y      The y coordinate to move the mouse to. If no x and y
	 *               coords are given, the current position is used.
	 ...
	 * @param button The button to click: "left", "right", "middle", "main",
	 *               "menu", "primary", "secondary".
	 * @param x1     The x coord to start the drag operation from.
	 * @param y1     The y coord to start the drag operation from.
	 * @param x2     The x coord to start the drag operation to.
	 * @param y2     The y coord to start the drag operation to.
	 * @param speed  the speed to move the mouse in the range 1 (fastest) to
	 *               100 (slowest). A speed of 0 will move the mouse instantly.
	 *               Default speed is 10.
	 * @return
	 ...
	 * @param point
	 */
    public void AU3_MouseGetPos(WinDef.POINT point);
	 ...
	 * can be seen (for win32 the contents of the Notepad window).<br>
	 ...
	 * @param value  The parameter (varies by option).
	 ...
	 * pixel is checked.
	 *
	 * @param rect position and size of rectangle
	 ...
	 */
    public int AU3_PixelChecksum(final WinDef.RECT rect, final Integer step);
	 ...
	 * @param shadeVariation A number between 0 and 255 to indicate the allowed number
	 ...
	 *                       check every other pixel. Default is 1.
	 * @param point          Return the pixel's coordinates if success, sets
	 *                       oAutoIt.error to 1 if color is not found.
	 */
    public void AU3_PixelSearch(WinDef.RECT rect, int color,
								Integer shadeVariation, Integer step, WinDef.POINT point);
	 ...
	 * @param timeout Specifies how long to wait (default is to wait
	 *                indefinitely).
	 ...
	 *
	 * @param keyName The registry key to delete.
    ...
    /**
	 * Reads the name of a subkey according to it's win32.
	 * <p/>
	 * A registry key must start with "HKEY_LOCAL_MACHINE" ("HKLM") or
	 * "HKEY_USERS" ("HKU") or "HKEY_CURRENT_USER" ("HKCU") or
	 * "HKEY_CLASSES_ROOT" ("HKCR") or "HKEY_CURRENT_CONFIG" ("HKCC").
	 ...
	 * @param instance The 1-based key win32 to retrieve.
	 * @param result
	 * @param bufSize
	 */
    ...
    /**
	 * Reads the name of a value according to it's win32.
	 * <p/>
	 * A registry key must start with "HKEY_LOCAL_MACHINE" ("HKLM") or
	 * "HKEY_USERS" ("HKU") or "HKEY_CURRENT_USER" ("HKCU") or
	 * "HKEY_CLASSES_ROOT" ("HKCR") or "HKEY_CURRENT_CONFIG" ("HKCC").
	 *
	 * @param keyName  The registry key to read.
	 * @param instance The 1-based value win32 to retrieve.
	 * @param result
	 * @param bufSize
	 */
	 ...
	 * @param type      Type of key to write: "REG_SZ", "REG_MULTI_SZ",
	 ...
	 * @param showFlag   The "show" flag of the executed program:
	 *                   <p/>
	 ...
	 * @param logonFlag  0 = do not load the user profile,<br/>
	 *                   1 = (default) load the user profile,<br/>
	 *                   2 = use for net credentials only
	 * @param fileName   The name of the executable (EXE, BAT, COM, or PIF) to run.
	 * @param workingDir Optional: The working directory.
	 * @param showFlag   Optional: The "show" flag of the executed program:<br/>
	 ...
	 *                  1 = (default) load the user profile,<br/>
	 *                  2 = use for net credentials only
	 * @param program   The name of the executable (EXE, BAT, COM, or PIF) to run.
	 * @param dir       Optional: The working directory.
	 * @param showFlag  Optional: The "show" flag of the executed program:<br/>
	 *                  SW_HIDE = Hidden window<br/>
	 *                  SW_MINIMIZE = Minimized window<br/>
	 *                  SW_MAXIMIZE = Maximized window
	 ...
	 * Ctrl+C, and so on.
	 *
	 * @param text The sequence of keys to send.
	 * @param flag Changes how "keys" is processed: flag = 0 (default), Text
	 ...
	 *
	 * @param text The text of the tooltip. (An empty string clears a
	 *             displaying tooltip).
	 ...
	 * @param text  The text of the window to close.
	 ...
	 * @param lpPoint
	 */
    public void AU3_WinGetCaretPos(WinDef.POINT lpPoint);
    ...
    public void AU3_WinGetClientSize(final WString title,
									 final WString text, WinDef.RECT rect);
	 ...
	 */
    public WinDef.HWND AU3_WinGetHandle(final WString title, final WString text);
	 ...
	 * @param title The title of the window to read.
	 * @param text  The text of the window to read.
	 * @param item1 Text of Menu Item
	 * @param item2 Text of SubMenu item
	 * @param item3 Text of SubMenu item
	 * @param item4 Text of SubMenu item
	 * @param item5 Text of SubMenu item
	 * @param item6 Text of SubMenu item
	 * @param item7 Text of SubMenu item
	 * @param item8 Text of SubMenu item
	 ...
	 * @param flag  Determines whether the window should have the "TOPMOST"
	 ...
	 * @param text  The text of the window to show.
	 * @param flags The "show" flag of the executed program:
	 *              <p/>
	 *              SW_HIDE = Hide window
	 *              <p/>
	 ...
	 *
	 * @param title    The title of the window to change.
	 * @param text     The text of the window to change.
	 ...
	 * @param title        The title of the window to change.
	 * @param text         The text of the window to change.
	 * @param transparency A number in the range 0 - 255. The larger the number, the
	 ...
	 * Checks to see if a specified window exists.
	 *
	 * @param hWnd The handle the window to check.
	 * @return Returns 1 if the window exists, otherwise returns 0. WinExist
	 * will return 1 even if a window is hidden.
	 */
    public int AU3_WinExistsByHandle(final WinDef.HWND hWnd);
	 ...
	 * criteria, the most recently active window is used.
	 *
	 * @param hWnd    The handle of the window to read.
	 * @param retText
	 * @param bufSize
	 */
    public void AU3_WinGetTitleByHandle(final WinDef.HWND hWnd,
										final CharBuffer retText, final int bufSize);
}
